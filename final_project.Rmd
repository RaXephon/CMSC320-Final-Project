...
#spring
ext_dat <- cp %>% 
  mutate(spring = ifelse(month >= "03" & month <= "05", day, NA)) %>% 
  mutate(summer = ifelse(month >= "06" & month <= "08", day, NA)) %>% 
  mutate(fall = ifelse(month >= "09" & month <= "11", day, NA)) %>% 
  mutate(winter = ifelse(month >= "12" || month <= "02", day, NA)) 
