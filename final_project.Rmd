...
ext_dat <- cp %>% 
  ...
  mutate(spring = day >= "01" & day <= "31") 

#summer
ext_dat <- cp %>% 
  filter(month >= "06" & month <= "08") %>%
  mutate(summer = day >= "01" & day <= "31")

#fall
ext_dat <- cp %>% 
  filter(month >= "09" & month <= "11") %>%
  mutate(fall = day >= "01" & day <= "31") 

#winter
ext_dat <- cp %>% 
  filter(month >= "12" || month <= "02") %>%
  mutate(winter = day >= "01" & day <= "31") 
...
def corr_plot(x, y, groups, xlabel, ylabel, title = "", color = 'blue'):
    ...
    # plot our raw data
    ax = sns.scatterplot(x, y, data = ext_dat, color = color)
...
# Each group in 'groups' is one participant.
# We calculate the mean of each participant in order to get an average over all of their dates for that day.
means = ext_dat.mean()

# Various lists we'll use in a second...
names = ['mag'] # Name of columns
expanded_names = ['depth'] # Full names of traits/cols

# For each of our 6 traits, we want to plot the trait data vs their partner's decision to match them (match success)
    corr_plot('dec_o', names[0], means, 'Match Success', expanded_names[0], "All Participants:")
