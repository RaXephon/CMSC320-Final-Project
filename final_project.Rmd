...
The entries for (season_depth, season_mag) are called p-values. To generalize, the smaller a p-value is, the more correlated the two variables are. We will hold our threshold as p < 0.05, indicating that there is a less than 1 in 20 chance of being wrong. As the correlation table shows, spring and winter have a high correlation with depth, while summer and fall are less significant.

While it is nice to know this data, it would also be nice to have a visual representation of this.
...
While analyzing earthquakes by season was a good exercise in data mutation and correlation testing, it didn't turn out to be very useful. Instead we will try another analysis on magnitude that will hopefully yield more useful findings. To do this, we turn to the `mag` and `depth` columns. 
